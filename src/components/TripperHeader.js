...
class TripperHeaderAndroid extends React.Component {
...
class TripperHeaderIOS extends React.Component {
  static height: number;
  props: Props;

  render() {
    const {leftItem, title, rightItem, foreground} = this.props;
    const titleColor = foreground === 'dark' ? TripperColors.darkText : 'white';
    const itemsColor = foreground === 'dark' ? TripperColors.lightText : 'white';
...
class ItemWrapperIOS extends React.Component {
  ...
  render() {
    ...
    if (layout !== 'icon' && title) {
      ...
      content = <Image source={icon} style={{width: 32, height: 32}}/>;
    }
...
const Header = Platform.OS === 'ios' ? TripperHeaderIOS : TripperHeaderAndroid;
